...
import { getdataSmartPhone } from "../../../system_redux/Slices/smartPhoneSlices";
import { useNavigate } from "react-router-dom";
import LodingPage from "./../../loding-error/LodingPage";

const SeeOurSmartPhone = () => {
  const { smartPhone, lodingSmartPhone } = useSelector(
    (state) => state.finshSmartPhone
  );
  const dispatch = useDispatch();
  const navigate = useNavigate();

  useEffect(() => {
    dispatch(getdataSmartPhone());
  }, []);
  return (
    <div>
      {lodingSmartPhone ? (
        ...
        <div className="px-[6em]">
          ...
          <div className="grid grid-cols-1  2xl:grid-cols-4 lg:grid-cols-3 md:ps-[0em] lg:ps-0 2xl:ps-0 sm:ps-[9em]  md:grid-cols-2 sm:grid-cols-1 gap-7 justify-center items-center   pb-11">
            {smartPhone.products &&
              smartPhone.products.map((data, index) => (
                <div
                  key={index}
                ...
                >
                  <figure className="flex-col ">
                    <span
                      ...
                      className="mb-[-4em] lg:start-[-5.8em] md:start-[-4.8em] start-[-6.5em] px-5 bg-green-500 text-white font-bold mt-6 p-2"
                    >
                      Smartphones
                    </span>
                    <img
...
export default SeeOurSmartPhone;
