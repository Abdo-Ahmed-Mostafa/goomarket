...
import { getdataSkincare } from "../../../system_redux/Slices/skincareSlice";
import { useNavigate } from "react-router-dom";
import LodingPage from "./../../loding-error/LodingPage";

const SeeOurSkincare = () => {
  const { skincare, lodingSkincare } = useSelector(
    (state) => state.finshSkincares
  );
  const dispatch = useDispatch();
  const navigate = useNavigate();

  useEffect(() => {
    dispatch(getdataSkincare());
  }, []);
      ...
      {lodingSkincare ? (
        ...
        <div className="px-[6em]">
          ...
          <div className="grid grid-cols-1  2xl:grid-cols-4 lg:grid-cols-3 md:ps-[0em] lg:ps-0 2xl:ps-0 sm:ps-[9em]  md:grid-cols-2 sm:grid-cols-1 gap-7 justify-center items-center   pb-11">
            {skincare.products &&
              skincare.products.map((data, index) => (
                <div
                  key={index}
