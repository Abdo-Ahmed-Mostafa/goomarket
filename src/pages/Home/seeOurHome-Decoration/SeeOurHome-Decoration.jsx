...
import { getdataHomeDecoration } from "../../../system_redux/Slices/home-decoration";
import { useNavigate } from "react-router-dom";
import LodingPage from "./../../loding-error/LodingPage";

const SeeOurHomeDecoration = () => {
  const { homeDecoration, lodingHomeDecoration } = useSelector(
    (state) => state.finshHomeDecoration
  );
  const dispatch = useDispatch();
  const navigate = useNavigate();

  useEffect(() => {
    dispatch(getdataHomeDecoration());
  }, []);
  return (
    <div>
      {lodingHomeDecoration ? (
        <LodingPage />
      ) : (
        <div className="px-[6em]">
          ...
          <div className="grid grid-cols-1  2xl:grid-cols-4 lg:grid-cols-3 md:ps-[0em] lg:ps-0 2xl:ps-0 sm:ps-[9em]  md:grid-cols-2 sm:grid-cols-1 gap-7 justify-center items-center   pb-11">
            {homeDecoration.products &&
              homeDecoration.products.map((data, index) => (
                <div
                  key={index}
                ...
                >
                  <figure className="flex-col ">
                    <span
                      ...
                      className="mb-[-4em] xl:start-[-5em] lg:start-[-4.2em] md:start-[-4.3em] start-[-5em] px-5 bg-green-500 text-white font-bold mt-6 p-2"
                    >
                      Home-Decoration
                    </span>
                    <img
