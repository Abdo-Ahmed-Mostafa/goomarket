...
import { getdataFurniture } from "../../../system_redux/Slices/furnitureSlices";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import LodingPage from "./../../loding-error/LodingPage";

const Furniture = () => {
  const { furniture, lodingFurniture } = useSelector(
    (state) => state.finshFurniture
  );
  const dispatch = useDispatch();
  const navigate = useNavigate();

  useEffect(() => {
    dispatch(getdataFurniture());
  }, []);
  return (
    <div>
      {lodingFurniture ? (
        ...
        <div className="px-[6em]">
          ...
          <div className="grid grid-cols-1  2xl:grid-cols-4 lg:grid-cols-3 md:ps-[0em] lg:ps-0 2xl:ps-0 sm:ps-[9em]  md:grid-cols-2 sm:grid-cols-1 gap-7 justify-center items-center   pb-11">
            {furniture.products &&
              furniture.products.map((data, index) => (
                <div
                  key={index}
