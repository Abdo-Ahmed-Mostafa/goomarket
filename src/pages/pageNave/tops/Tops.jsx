...
import { getdataTops } from "../../../system_redux/Slices/topsSlices";
import { useNavigate } from "react-router-dom";
import LodingPage from "../../loding-error/LodingPage";

const Tops = () => {
  const { tops, lodingTops } = useSelector((state) => state.finshTops);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  useEffect(() => {
    dispatch(getdataTops());
  }, []);
  return (
    <div>
      {lodingTops ? (
